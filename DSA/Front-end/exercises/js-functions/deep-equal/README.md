...
Implement **Deep Equal** as described: Implement a function that determines if two values are equal
...
- `NaN` handling
- Different key order in objects
- Prototype chain ignored or not
- Nested structures
...
```bash
npm run test:exercise -- js-functions/deep-equal
```
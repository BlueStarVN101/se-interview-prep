...
Implement **Cycle** as described: Implement a function that takes one or more values and returns a function that cycles through those values each time it is called
...
```bash
npm run test:exercise -- js-functions/cycle
```
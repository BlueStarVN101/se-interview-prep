...
Implement **Sum** as described: Implement a function that sums numbers by accepting a number and allows for repeated calling with more numbers until it is not called with any number
...
```bash
npm run test:exercise -- js-functions/sum
```
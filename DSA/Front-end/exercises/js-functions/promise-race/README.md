...
Implement **Promise.race** as described: Implement the Promise.race () function that resolves or rejects when any of the input elements are resolved or rejected
...
```bash
npm run test:exercise -- js-functions/promise-race
```
...
Implement **Depth-first Search** as described: Implement a depth-first search algorithm that traverses a directed graph in a depth-first manner
...
```bash
npm run test:exercise -- algo-coding/depth-first-search
```
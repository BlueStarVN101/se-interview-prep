...
Implement **Maximum Sum in Contiguous Array** as described: Implement a function to find the subarray with the maximum sum
...
- Kadane's algorithm — track current and global max while scanning.
...
```bash
npm run test:exercise -- algo-coding/maximum-sum-in-contiguous-array
```
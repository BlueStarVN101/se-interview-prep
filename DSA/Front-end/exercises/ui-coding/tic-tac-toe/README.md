...
Build **Tic-tac-toe** as described: Build a tic-tac-toe game that is playable by two players
...
Walk through the primary user journey for **Tic-tac-toe** step by step in your README notes as you implement.
...
**Core interaction:** User performs the main action described in the problem (Build a tic-tac-toe game that is playable by two players).
...
```bash
npm run test:exercise -- ui-coding/tic-tac-toe
```
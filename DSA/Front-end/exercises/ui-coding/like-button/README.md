...
Build **Like Button** as described: Build a Like button that changes appearance based on its state
...
Walk through the primary user journey for **Like Button** step by step in your README notes as you implement.
...
**Core interaction:** User performs the main action described in the problem (Build a Like button that changes appearance based on its state).
...
```bash
npm run test:exercise -- ui-coding/like-button
```
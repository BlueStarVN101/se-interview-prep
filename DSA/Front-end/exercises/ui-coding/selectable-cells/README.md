...
Build **Selectable Cells** as described: Build an interface where users can drag to select multiple cells within a grid
...
Walk through the primary user journey for **Selectable Cells** step by step in your README notes as you implement.
...
**Core interaction:** User performs the main action described in the problem (Build an interface where users can drag to select multiple cells within a grid).
...
```bash
npm run test:exercise -- ui-coding/selectable-cells
```